...
{
    public class ClassicFireMage : CombatRoutine
    {
        //Spell Strings
       ...
       // private string RemoveCurse = "Remove Curse";
        ...
        private string FV = "Fire Vulnerability";
        private string Fireball = "Fireball";
        private string FBRange = "Fireball Range";
        //Talents
        ...
        private string UseComb => CDUsage[CombatRoutine.GetPropertyInt(Combustion)];
        ...
        private bool IsStack => API.ToggleIsEnabled("Scorch Stack");

        private bool FBRangeTalent => CombatRoutine.GetPropertyBool(FBRange);
        ...
        private bool InFireBallRange => FBRangeTalent ? API.TargetRange < 42 : API.TargetRange < 37;
        ...
        {
            CombatRoutine.Name = "Classic Fire Mage by Ryu";
      ...
      //      CombatRoutine.AddDebuff(Sated);
            ...
            CombatRoutine.AddDebuff(FV, 22959);

            //Spell
            CombatRoutine.AddSpell(Frostbolt, 10180);
            CombatRoutine.AddSpell(IB, 45438);
            CombatRoutine.AddSpell(IL, 30455);
            CombatRoutine.AddSpell(Fireball, 10149);
            CombatRoutine.AddSpell(Combustion, 11129);
            CombatRoutine.AddSpell(Scorch, 27074);
     ...
     //       CombatRoutine.AddSpell(RemoveCurse);
           ...
           // CombatRoutine.AddMacro(Spellsteal + "MO");
            ...
            CombatRoutine.AddToggle("Scorch Stack");
            ...
            CombatRoutine.AddProp(FBRange, FBRange, false, "Do you have Fireball Range Talent?", "Fireball");
    ...
    //            {
      //              for (int i = 0; i < CurseList.Length; i++)
        ...
        private void rotation()
        {
            ...
            {
                if (API.CanCast(CoC) && API.TargetRange < 6 && !API.PlayerIsMoving) 
                {
                    API.CastSpell(CoC);
                    return;
                }
            ...
            if (NotChanneling && !API.PlayerSpellonCursor && InRange && IsNotEatingDrink)
            {
                ...
                if (API.CanCast(IV) && !API.SpellISOnCooldown(IV) && (UseIV == "With Cooldowns" && IsCooldowns || UseIV == "On Cooldown" && API.TargetDebuffStacks(FV) == 5) && !API.PlayerHasBuff(IV) && !API.PlayerIsCasting())
                {
                    API.CastSpell(IV);
                    return;
                }
                if (API.CanCast(CS) && !API.SpellISOnCooldown(CS) && API.SpellISOnCooldown(IV) && (UseCS == "With Cooldowns" && IsCooldowns || UseCS == "On Cooldown")  && !API.PlayerIsCasting())
                {
                    API.CastSpell(CS);
                    return;
                }
                if (API.CanCast(Combustion) && !API.SpellISOnCooldown(Combustion) &&  (UseComb == "With Cooldowns" && IsCooldowns || UseComb == "On Cooldown" && API.TargetDebuffStacks(FV) == 5) && !API.PlayerIsCasting())
                {
                    API.CastSpell(Combustion);
                    return;
                }
                if (API.CanCast(Scorch) && InRange && IsStack && (API.TargetDebuffStacks(FV) < 5 || API.TargetDebuffRemainingTime(FV, false) <= 250) && !API.PlayerIsCasting())
                {
                    API.CastSpell(Scorch);
                    return;
                }
                if (API.CanCast(Fireball) && InFireBallRange && !API.PlayerIsCasting())
                {
